...
for i, y2 in enumerate(np.linspace(1.25, -2.25, 4)):
    ...
    ax.text(x2, y2, r'$f(\theta)$', fontsize=25, ha='center', va='center')
...
ax.text(x3, y3, r'$g(\theta)$', fontsize=25, ha='center', va='center')
    ...
    for j, y2 in enumerate(np.linspace(1.25, -2.25, 4)):
        ...
        if i in [0, 4]:
            ax.text((2*x1+x2)/3, (2*y1+y2)/3 + shift,
                    '$w_{%s%s}$' % ((i+1), (j+1)), va=va, fontsize=25,
                    ha='center')
...
plt.text(x2, 2.7, r"Hidden Layer",
         ha='center', va='top')
plt.text(x3, 2.7, r"Output Layer",
         ha='center', va='top')
